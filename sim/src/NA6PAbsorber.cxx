...
void NA6PAbsorber::createGeometry(TGeoVolume *world)
{
  ...
  for (int isl=0;isl<param.nAbsorberSlices;isl++) {
    if (isl == param.nAbsorberSlices-1 && param.posMuonWall != 0) { // special setting for last slice (muon wall)
    ...
    if (param.radPlug[isl]>0 && !param.medAbsorberPlug[isl].empty()) {
      ...
      world->AddNode(plugVol, isl + 1, new TGeoTranslation(0.0, 0.0, zplace + param.thicknessAbsorber[isl]/2));
    }
    auto sliceVol = new TGeoVolume(slnm.c_str(), slice, NA6PTGeoHelper::instance().getMedium(param.medAbsorber[isl]));
    world->AddNode(sliceVol, isl + 1, new TGeoTranslation(0.0, 0.0, zplace + param.thicknessAbsorber[isl]/2));
