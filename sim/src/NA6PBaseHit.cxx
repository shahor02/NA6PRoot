...
NA6PBaseHit::NA6PBaseHit(int trackid, short did, const TVector3& xyzIn, const TVector3& xyzOut, const TVector3& momIn, float time, float val, uint8_t statusStart, uint8_t statusEnd) :
  mTime(time), mHitValue(val), mTrackID(trackid), mDetectorID(did), mTrackStatusStart(statusStart), mTrackStatusEnd(statusEnd)
{
  setPosIn(xyzIn);
  setPosOut(xyzOut);
  setMomIn(momIn);
