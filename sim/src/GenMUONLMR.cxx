                                                                ...
                                                                fHNProc(0x0)
{
  ...
  for (Int_t index = 0; index < 3; index++)
  {
    ...
    for (Int_t ibin = 0; ibin < nbins; ibin++)
    {
      ...
      if (xd > 4. * epsilon)
      {
        Double_t bracket = TMath::Power(1. + xd / (1. - delta), 2) - 4. * xd / ((1. - delta) * (1. - delta));
        if (bracket > 0)
        {
...
GenMUONLMR::GenMUONLMR(GenMUONLMR &gen) : fNMuMin(gen.fNMuMin),
                                          ...
                                          fHNProc(gen.fHNProc)
{
  ...
  for (Int_t i = 0; i < 2; i++)
    fDecay[i] = (TF1 *)gen.fDecay[i]->Clone();
  for (Int_t i = 0; i < 3; i++)
    fDalitz[i] = (TH1F *)gen.fDalitz[i]->Clone();
  for (Int_t i = 0; i < 2; i++)
    fMu[i] = (TParticle *)gen.fMu[i]->Clone();
}
...
void GenMUONLMR::Decay2Body(TParticle *mother)
{
  ...
  if (pdg == 113)
    mres = fRhoLineShape->GetRandom();
  else
    mres = mother->GetCalcMass();
...
void GenMUONLMR::DecayPiK(TParticle *mother, Bool_t &hasDecayed)
{
  ...
  TVector3 betaParent = (1. / vmother.E()) * vmother.Vect(); // beta = p/E
  v1.Boost(betaParent);
  if (mother->GetPdgCode() > 0)
    fMu[0]->SetMomentum(v1.Px(), v1.Py(), v1.Pz(), v1.E());
  else
    fMu[1]->SetMomentum(v1.Px(), v1.Py(), v1.Pz(), v1.E());

  Int_t idmother = -1;
  if (TMath::Abs(mother->GetPdgCode()) == 211)
    idmother = 0;
  if (TMath::Abs(mother->GetPdgCode()) == 321)
    idmother = 1;
  Double_t gammaRes = mother->Energy() / mres;
  Double_t zResCM = fDecay[idmother]->GetRandom();
  Double_t zResLab = gammaRes * zResCM;
  if (zResLab > 0.938)
    hasDecayed = 0; // 0.938: distance from IP to absorber + lambda_i
  else
    hasDecayed = 1;
}
...
void GenMUONLMR::DalitzDecay(TParticle *mother)
{
  ...
  if (mother->GetPdgCode() == 223)
    md3 = 0.134977; // if mother is an omega, third particle is a pi0
  Int_t index = -1;
  if (mother->GetPdgCode() == 221)
    index = 0; // eta
  else if (mother->GetPdgCode() == 223)
    index = 1; // omega
  ...
  while (flag == 0)
  {
    ...
    // check kinematics
    if (mres - md3 > TMath::Sqrt(mvirt2) && TMath::Sqrt(mvirt2) / 2. > mumass)
      flag = 1;
    if (++countIt > 1E11)
    {
...
Double_t GenMUONLMR::FormFactor(Double_t q2, Int_t decay)
{
  ...
  case 0: // eta -> mu mu gamma
    ...
    if (q2 < mass2)
      ff2 = TMath::Power(1. / (1. - lambda2inv * q2), 2);
    else
      ff2 = 0;
    break;
  ...
  case 2: // etaPrime -> mu mu gamma
    ...
    m2 = 0.1020 * 0.1020;
    if (q2 < mass2)
      ff2 = n4 / (TMath::Power(n2 - q2, 2) + m2 * n2);
    else
      ff2 = 0;
    break;
  default:
...
Double_t GenMUONLMR::RhoLineShapeNew(Double_t *x, Double_t *para)
{
  ...
  r = pow(mass, 1.5) * pow((1 - mThreshold * mThreshold / (mass * mass)), 1.5) *
      ((mass * mass + 2 * mMu * mMu) / (mass * mass)) * (pow((mass * mass - 4 * mMu * mMu), 0.5) / mass) * FormFactor2 * exp(-mass / T) / Norm;
