...
void ConfigurableParam::writeINI(std::string const& filename, std::string const& keyOnly)
{
  ...
  auto outfilename = na6p::utils::Str::concat_string(sOutputDir, filename);
...
{
  return std::filesystem::exists(na6p::utils::Str::concat_string(ConfigurableParamReaders::getInputDir(), filepath));
}

// ------------------------------------------------------------------

void ConfigurableParam::setValue(std::string const& key, std::string const& valuestring, EParamProvenance p)
{
...
void ConfigurableParam::updateFromFile(std::string const& configFile, std::string const& paramsList, bool unchangedOnly)
{
  ...
  auto cfgfile = na6p::utils::Str::trim_copy(configFile);
  ...
  auto request = na6p::utils::Str::tokenize(paramsList, ',', true);
...
void ConfigurableParam::updateFromString(std::string const& configString)
{
  ...
  auto cfgStr = na6p::utils::Str::trim_copy(configString);
  if (cfgStr.length() == 0) {
    return;
  }
...
*/
  ...
  auto params = na6p::utils::Str::tokenize(configString, ';', true);

  // Now split each key=value string into its std::pair<key, value> parts
  auto keyValues = toKeyValPairs(params);

  setValues(keyValues, kRT);

  const auto& kv = na6p::conf::KeyValParam::Instance();
  if (getProvenance("keyval.input_dir") != kCODE) {
    ConfigurableParamReaders::setInputDir(na6p::utils::Str::concat_string(na6p::utils::Str::rectifyDirectory(kv.input_dir)));
  }
  if (getProvenance("keyval.output_dir") != kCODE) {
    if (kv.output_dir == "/dev/null") {
      sOutputDir = kv.output_dir;
    } else {
      sOutputDir = na6p::utils::Str::concat_string(na6p::utils::Str::rectifyDirectory(kv.output_dir));
    }
  }
}
...
void ConfigurableParam::setValues(std::vector<std::pair<std::string, std::string>> const& keyValues, EParamProvenance p)
{
  ...
  for (auto& keyValue : keyValues) {
    ...
    std::string value = na6p::utils::Str::trim_copy(keyValue.second);
    ...
    if (sEnumRegistry->contains(key)) {
      setEnumValue(key, value, p);
    } else if (isArray(value)) {
      setArrayValue(key, value, p);
    } else {
      ...
      setValue(key, value, p);
    }
  }
}

void ConfigurableParam::setArrayValue(const std::string& key, const std::string& value, EParamProvenance p)
{
  // We remove the lead/trailing square bracket
  // value.erase(0, 1).pop_back();
  auto elems = na6p::utils::Str::tokenize(value.substr(1, value.length() - 2), ',', true);
  ...
  for (int i = 0; i < elems.size(); ++i) {
    ...
    setValue(indexKey, elems[i], p);
  }
}

void ConfigurableParam::setEnumValue(const std::string& key, const std::string& value, EParamProvenance p)
{
  ...
  setValue(key, std::to_string(val), p);
}
