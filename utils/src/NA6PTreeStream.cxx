...
NA6PTreeSRedirector::NA6PTreeSRedirector(const char *fname,const char * option) :
  fDirectory(NULL),
...
//-------------------------------------------------------------
NA6PTreeDataElement:: NA6PTreeDataElement(Char_t type) :
  TNamed(),
  fType(type),
  fDType(0),
  fClass(0),
...
NA6PTreeDataElement:: NA6PTreeDataElement(TDataType* type) :
  TNamed(),
  fType(0),
  fDType(type),
  fClass(0),
...
NA6PTreeDataElement:: NA6PTreeDataElement(TClass* cl) :
  TNamed(),
  fType(0),
  fDType(0),
  fClass(cl),
...
//-------------------------------------------------------------------
NA6PTreeStream::NA6PTreeStream(const char *treename, TTree* externalTree):
...
void NA6PTreeStream::Close()
{
  ...
  //
  if (NA6PTreeSRedirector::IsDisabled()) return;
  fTree->Write();
}
...
Int_t NA6PTreeStream::CheckIn(Char_t type, void *pointer)
{
  ...
  //
  if (NA6PTreeSRedirector::IsDisabled()) return 0;
...
NA6PTreeStream & NA6PTreeStream::Endl()
{
  ...
  //
  if (NA6PTreeSRedirector::IsDisabled()) return *this;
