...
void NA6PTreeStream::Test()
{
  //
  //
  TFile* ftest = new TFile("teststreamer.root", "recreate");
  if (!ftest)
    ftest = new TFile("teststreamer.root", "new");
  //
  // create to streems Tree1 and Tree2
  NA6PTreeStream stream1("Tree1");
  NA6PTreeStream stream2("Tree2");
  //
  Char_t ch = 's';
  Float_t f = 3.;
  Float_t f2 = 1;
...
void NA6PTreeSRedirector::Test2()
{
  //
  // Example test function to show functionality of NA6PTreeSRedirector
  //
  //
  ...
  NA6PTreeSRedirector* pmistream = new NA6PTreeSRedirector();
  NA6PTreeSRedirector& mistream = *pmistream;
  Char_t ch = 's';
  Float_t f = 3.;
  Float_t f2 = 1;
  TObject* po = new TObject;
...
void NA6PTreeSRedirector::Test()
{
  ...
  //
  //
  NA6PTreeSRedirector* pmistream = new NA6PTreeSRedirector("testredirector.root");
  NA6PTreeSRedirector& mistream = *pmistream;
  Char_t ch = 's';
  Float_t f = 3.;
  Float_t f2 = 1;
...
void NA6PTreeSRedirector::UnitTest(Int_t testEntries)
{
  //
  //
  //
...
void NA6PTreeSRedirector::UnitTestSparse(Double_t scale, Int_t testEntries)
{
  //
  // Unit test for the NA6PTreeSRedirector
  ...
  if (scale > 1)
    scale = 1;
  NA6PTreeSRedirector* pcstream = new NA6PTreeSRedirector("testpcstreamSparse.root", "recreate");
  ...
  if (!f) {
    printf("FAILED: file: %p, NA6PTreeSRedirector::IsDisabled()=%i\n", f, NA6PTreeSRedirector::IsDisabled() ? 1 : 0);
    return;
  }
  ...
  printf("#UnitTest:\tNA6PTreeSRedirector::TestSparse(%f)\tRatioSkip\t%f\n", scale, ratio);
  printf("#UnitTest:\tNA6PTreeSRedirector::TestSparse(%f)\tRatioSkip0\t%f\n", scale, ratio0);
  printf("#UnitTest:\tNA6PTreeSRedirector::TestSparse(%f)\tRatioZerro\t%f\n", scale, ratio1);
  //    b.) Integrity
  ...
  printf("#UnitTest:\tNA6PTreeSRedirector::TestSparse(%f)\tOutlyersSkip\t%d\n", scale, outlyersSparseSkip != 0);
  printf("#UnitTest:\tNA6PTreeSRedirector::TestSparse(%f)\tOutlyersSkip0\t%d\n", scale, outlyersSparseSkip0 != 0);
  //    c.) Number of entries
  //
  Int_t entries = treeFull->GetEntries();
  Int_t entries0 = treeSparseSkip0->GetEntries();
  Bool_t isOKStat = (entries == entries0);
  printf("#UnitTest:\tNA6PTreeSRedirector::TestSparse(%f)\tEntries\t%d\n", scale, isOKStat);
  //
  //   d.)Reading test
  TVectorD* pvecRead = 0;
  ...
  printf("#UnitTest:\tNA6PTreeSRedirector::TestSparse(%f)\tReadOK\t%d\n", scale, readOK);
  //
  //   e.)Global test
  Bool_t isOK = (outlyersSparseSkip0 == 0) && isOKStat && readOK;
  printf("#UnitTest:\tNA6PTreeSRedirector::TestSparse(%f)\tisOk\t%d\n", scale, isOK);
}

Bool_t NA6PTreeSRedirector::fgDisabled = kFALSE;
NA6PTreeSRedirector::NA6PTreeSRedirector(const char* fname, const char* option) : fDirectory(NULL),
...
NA6PTreeSRedirector::~NA6PTreeSRedirector()
{
  //
  // Destructor
  //
...
void NA6PTreeSRedirector::StoreObject(TObject* object)
{
  //
  //
  //
...
void NA6PTreeSRedirector::SetDirectory(TDirectory* sfile)
{
  //
...
NA6PTreeStream& NA6PTreeSRedirector::operator<<(Int_t id)
{
  //
  // return reference to the data layout with given identifier
  // if not existing - creates new
  if (!fDataLayouts)
    fDataLayouts = new TObjArray(fgDisabled ? 1 : 10000);
  NA6PTreeStream* clayout = 0;
  Int_t entries = fDataLayouts->GetEntriesFast();
  for (Int_t i = 0; i < entries; i++) {
    NA6PTreeStream* layout = (NA6PTreeStream*)fDataLayouts->At(i);
    if (!layout)
      continue;
  ...
  if (!clayout) {
    ...
    clayout = new NA6PTreeStream(chname);
    clayout->fId = id;
...
void NA6PTreeSRedirector::SetExternalTree(const char* name, TTree* externalTree)
{
  NA6PTreeStream* clayout = (NA6PTreeStream*)fDataLayouts->FindObject(name);

  if (!clayout) {
    TDirectory* backup = gDirectory;
    fDirectory->cd();
    clayout = new NA6PTreeStream(name, externalTree);
    clayout->fId = -1;
...
NA6PTreeStream& NA6PTreeSRedirector::operator<<(const char* name)
{
  //
  // return reference to the data layout with given identifier
  // if not existing - creates new
  if (!fDataLayouts)
    fDataLayouts = new TObjArray(10000);
  NA6PTreeStream* clayout = (NA6PTreeStream*)fDataLayouts->FindObject(name);
  Int_t entries = fDataLayouts->GetEntriesFast();
  ...
  if (!clayout) {
    TDirectory* backup = gDirectory;
    fDirectory->cd();
    clayout = new NA6PTreeStream(name);
    clayout->fId = -1;
    clayout->SetName(name);
    fDataLayouts->AddAt(clayout, entries);
    if (backup)
...
void NA6PTreeSRedirector::Close()
{
  //
  //
  TDirectory* backup = gDirectory;
  ...
  if (fDataLayouts) {
    ...
    for (Int_t i = 0; i < entries; i++) {
      ...
      if (layout && !fgDisabled) {
        if (layout->fTree)
          layout->fTree->Write(layout->GetName());
      }
    }
    delete fDataLayouts;
    fDataLayouts = 0;
...
//-------------------------------------------------------------
NA6PTreeDataElement::NA6PTreeDataElement(Char_t type) : TNamed(),
                                                        fType(type),
                                                        fDType(0),
                                                        fClass(0),
                                                        fPointer(0)
...
NA6PTreeDataElement::NA6PTreeDataElement(TDataType* type) : TNamed(),
                                                            fType(0),
                                                            fDType(type),
                                                            fClass(0),
                                                            fPointer(0)
...
NA6PTreeDataElement::NA6PTreeDataElement(TClass* cl) : TNamed(),
                                                       fType(0),
                                                       fDType(0),
                                                       fClass(cl),
                                                       fPointer(0)
...
//-------------------------------------------------------------------
NA6PTreeStream::NA6PTreeStream(const char* treename, TTree* externalTree) : TNamed(treename, treename),
                                                                            fElements(0),
                                                                            fBranches(0),
                                                                            fTree(externalTree),
                                                                            fCurrentIndex(0),
...
void NA6PTreeStream::Close()
{
  //
  // Flush data to disk and close
  //
  if (NA6PTreeSRedirector::IsDisabled())
    return;
  fTree->Write();
}

Int_t NA6PTreeStream::CheckIn(Char_t type, void* pointer)
{
  //
  ...
  if (NA6PTreeSRedirector::IsDisabled())
    return 0;
  if (!fElements)
    fElements = new TObjArray(10000);
  if (fElements->GetSize() <= fCurrentIndex)
    fElements->Expand(fCurrentIndex * 2);
  NA6PTreeDataElement* element = (NA6PTreeDataElement*)fElements->At(fCurrentIndex);
  if (!element) {
    element = new NA6PTreeDataElement(type);
    //
    char name[1000];
...
Int_t NA6PTreeStream::CheckIn(TObject* pObject)
{
  //
  // Insert TObject
  //
  ...
  if (!fElements) {
    fElements = new TObjArray(1000);
  }
  NA6PTreeDataElement* element = (NA6PTreeDataElement*)fElements->At(fCurrentIndex);
  if (!element) {
    element = new NA6PTreeDataElement(pClass);
    //
    char name[1000];
    if (fNextName.Length() > 0) {
      if (fNextNameCounter == 0) {
...
void NA6PTreeStream::BuildTree()
{
  ...
  for (Int_t i = 0; i < entries; i++) {
    //
    NA6PTreeDataElement* element = (NA6PTreeDataElement*)fElements->At(i);
    if (fBranches->At(i))
      continue;
    char bname1[1000];
    ...
    if (element->fClass) {
      if (element->fClass->GetBaseClass("TClonesArray")) {
        ...
        if (entriesFilled != 0) {
          br->SetAddress(0);
          for (Int_t ientry = 0; ientry < entriesFilled; ientry++)
            br->Fill();
...
void NA6PTreeStream::Fill()
{
  ...
  if (fTree) {
    Int_t entries = fElements->GetEntriesFast();
    if (entries > fTree->GetNbranches())
      BuildTree();
    for (Int_t i = 0; i < entries; i++) {
      NA6PTreeDataElement* el = (NA6PTreeDataElement*)fElements->At(i);
      if (!el)
        continue;
      if (!el->GetType())
        continue;
      ...
      if (br && el) {
        if (el->GetType())
          br->SetAddress(el->fPointer);
      }
    }
    if (fStatus == 0)
      fTree->Fill(); // fill only in case of non conflicts
    fStatus = 0;
  }
}
...
void NA6PTreeSRedirector::FixLeafNameBug(TTree* tree)
{
