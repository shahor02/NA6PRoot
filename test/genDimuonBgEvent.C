...
bool getParams(float& T_piM, float& y0_piM, float& ysig_piM, float& dNdY_piM,
	       ...
	       float& y0_Jpsi, float& yboxhalfw_Jpsi, float& ysig_Jpsi, float& pt1_Jpsi, float& pt2_Jpsi, float& pt3_Jpsi,
	       float& y0SG, float& sigySG, float& TSG_Phi, float& TSG_Omega, float& MotherMass_Phi, float& MotherMass_Omega)
{
...
NA6PGenerator* addBgEventGenerator(NA6PGenCocktail* genCocktail, int NSignalinAcc, float ptMin, float ptMax, float yMin, float yMax, bool Bg, const char *Part)
{
  ...
  float y0_Jpsi = 0., yboxhalfw_Jpsi = 0, ysig_Jpsi = 0., pt1_Jpsi = 0., pt2_Jpsi = 0., pt3_Jpsi = 0.;
  float y0SG = 0., sigySG = 0., TSG_Phi = 0., TSG_Omega = 0., MotherMass_Phi = 0, MotherMass_Omega = 0;
  ...
  getParams(T_piM, y0_piM, ysig_piM, dNdY_piM,
	    ...
	    y0_Jpsi, yboxhalfw_Jpsi, ysig_Jpsi, pt1_Jpsi, pt2_Jpsi, pt3_Jpsi,
	    y0SG, sigySG, TSG_Phi, TSG_Omega, MotherMass_Phi, MotherMass_Omega);
...
if(strcmp(Part,"Jpsi")==0){
  ...
  genJpsi->setParametersTrans({pt1_Jpsi, pt2_Jpsi, pt3_Jpsi}); //pT
  genJpsi->setParametersLong({y0_Jpsi, yboxhalfw_Jpsi, ysig_Jpsi}); //y
//  if(SignalinAcc){
    genJpsi->setMultiplicity(NSignalinAcc); // to generate a fixed number of signal in acceptance, setPoisson = false
...
//  }  
  ...
  genPhi->setParametersTrans({TSG_Phi, MotherMass_Phi}); //pT
  genPhi->setParametersLong({y0SG, sigySG}); //y
//  if(SignalinAcc){
...
//  }  
  ...
  genOmega->setParametersTrans({TSG_Omega, MotherMass_Omega}); //pT
  genOmega->setParametersLong({y0SG, sigySG}); //y
//  if(SignalinAcc){
...
NA6PGenerator* genDimuonBgEvent(int nSignalinAcc=1, const char *Part= "Jpsi", float ptMin = 0., float ptMax = 5., float yMin = -2.5, float yMax = 4.5, bool Bg = false)
{
