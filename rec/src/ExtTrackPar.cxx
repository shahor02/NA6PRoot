// RS: this is a standalone version of AliRoot AliExternalTrackParam
...
ClassImp(ExtTrackPar)

Double32_t ExtTrackPar::fgMostProbablePt=kMostProbablePt;
Bool_t ExtTrackPar::fgUseLogTermMS = kFALSE;; 
//_____________________________________________________________________________
ExtTrackPar::ExtTrackPar() :
  fX(0),
  fAlpha(0)
{
  //
...
//_____________________________________________________________________________
ExtTrackPar::ExtTrackPar(const ExtTrackPar &track):
  TObject(track),
  fX(track.fX),
  fAlpha(track.fAlpha)
{
...
//_____________________________________________________________________________
ExtTrackPar& ExtTrackPar::operator=(const ExtTrackPar &trkPar)
{
  //
  // assignment operator
  //
...
//_____________________________________________________________________________
ExtTrackPar::ExtTrackPar(Double_t x, Double_t alpha, 
...
//_____________________________________________________________________________
ExtTrackPar::ExtTrackPar(Double_t xyz[3],Double_t pxpypz[3],
...
/*
//_____________________________________________________________________________
void ExtTrackPar::set(Double_t xyz[3],Double_t pxpypz[3],
				Double_t cv[21],Short_t sign) 
{
  //
...
//_____________________________________________________________________________
void ExtTrackPar::AddCovariance(const Double_t c[15]) {
  //
...
Double_t ExtTrackPar::getP() const {
  //---------------------------------------------------------------------
...
Double_t ExtTrackPar::get1P() const {
  //---------------------------------------------------------------------
...
Double_t ExtTrackPar::getD(Double_t x,Double_t y,Double_t b) const {
  //------------------------------------------------------------------
...
//_______________________________________________________________________
Double_t ExtTrackPar::getLinearD(Double_t xv,Double_t yv) const {
  //------------------------------------------------------------------
  // This function calculates the transverse impact parameter
...
Bool_t ExtTrackPar::correctForMeanMaterialdEdx
...
Bool_t ExtTrackPar::correctForMeanMaterialZA
(Double_t xOverX0, Double_t xTimesRho, Double_t mass,
 Double_t zOverA,
 Double_t density,
 Double_t exEnergy,
...
Bool_t ExtTrackPar::correctForMaterial
...
Double_t ExtTrackPar::BetheBlochAleph(Double_t bg,
         Double_t kp1,
         Double_t kp2,
         Double_t kp3,
         Double_t kp4,
...
Double_t ExtTrackPar::BetheBlochGeant(Double_t bg,
         Double_t kp0,
         Double_t kp1,
         Double_t kp2,
         Double_t kp3,
...
Double_t ExtTrackPar::BetheBlochSolid(Double_t bg) {
  //------------------------------------------------------------------
...
Double_t ExtTrackPar::BetheBlochGas(Double_t bg) {
  //------------------------------------------------------------------
  // This is an approximation of the Bethe-Bloch formula, 
...
Bool_t ExtTrackPar::Rotate(Double_t alpha) {
  //------------------------------------------------------------------
  ...
  if (TMath::Abs(fP[2]) >= kAlmost1) {
    printf("Error: Precondition is not satisfied: |sin(phi)|>1 ! %f\n",fP[2]); 
     return kFALSE;
  }
  ...
  if (TMath::Abs(tmp) >= kAlmost1) {
    //     if (TMath::Abs(tmp) > 1.+ Double_t(FLT_EPSILON)) AliWarning(Form("Rotation failed ! %.10e",tmp));
     return kFALSE;
  }
  fAlpha = alpha;
  fX =  x*ca + fP0*sa;
  ...
  if (TMath::Abs(cf)<kAlmost0) {
    printf("Error: Too small cosine value %f\n",cf); 
    cf = kAlmost0;
  } 
...
Bool_t ExtTrackPar::RotateParamOnly(Double_t alpha)
{
...
Bool_t ExtTrackPar::propagateTo(Double_t xk, Double_t b) {
  //----------------------------------------------------------------
...
Bool_t ExtTrackPar::propagateParamOnlyTo(Double_t xk, Double_t b) {
  //----------------------------------------------------------------
  // propagate this track to the plane X=xk (cm) in the field "b" (kG)
...
Bool_t 
ExtTrackPar::propagate(Double_t alpha, Double_t x, Double_t b) {
  //------------------------------------------------------------------
  // Transform this track to the local coord. system rotated
  // by angle "alpha" (rad) with respect to the global coord. system, 
...
Bool_t ExtTrackPar::propagateBxByBz
...
void ExtTrackPar::propagate(Double_t len, Double_t x[3],
...
Bool_t ExtTrackPar::Intersect(Double_t pnt[3], Double_t norm[3],
Double_t bz) const {
  //+++++++++++++++++++++++++++++++++++++++++    
  // Origin: K. Shileev (Kirill.Shileev@cern.ch)
...
Double_t 
ExtTrackPar::getPredictedChi2(const Double_t p[2],const Double_t cov[3]) const {
  //----------------------------------------------------------------
...
Double_t ExtTrackPar::
getPredictedChi2(const ExtTrackPar *t) const {
  //----------------------------------------------------------------
  ...
  if (TMath::Abs(t->getAlpha()-getAlpha()) > FLT_EPSILON) {
      printf("Error: The reference systems of the tracks differ !\n");
      return kVeryBig;
  }
  if (TMath::Abs(t->getX()-getX()) > FLT_EPSILON) {
      printf("Error: The reference of the tracks planes differ !\n");
      return kVeryBig;
  }
...
Double_t *ExtTrackPar::getResiduals(
...
Bool_t ExtTrackPar::Update(const Double_t p[2], const Double_t cov[3]) {
  //------------------------------------------------------------------
...
void 
ExtTrackPar::getHelixParameters(Double_t hlx[6], Double_t b) const {
  //--------------------------------------------------------------------
...
Double_t ExtTrackPar::getDCA(const ExtTrackPar *p, 
Double_t b, Double_t &xthis, Double_t &xp) const {
  ...
  while (max--) {
     ...
     if (TMath::Abs(dt2)/(TMath::Abs(t2)+1.e-3) < 1.e-4) {
	  ...
	  printf(" stopped at not a stationary point !\n");
        Double_t lmb=h11+h22; lmb=lmb-TMath::Sqrt(lmb*lmb-4*det);
        if (lmb < 0.) 
	  printf(" stopped at not a minimum !\n");
        break;
     }
...
Double_t ExtTrackPar::
propagateToDCA(ExtTrackPar *p, Double_t b) {
  //--------------------------------------------------------------
...
void ExtTrackPar::getDirection(Double_t d[3]) const {
  //----------------------------------------------------------------
...
Bool_t ExtTrackPar::getPxPyPz(Double_t p[3]) const {
  //---------------------------------------------------------------------
...
Double_t ExtTrackPar::PhiPos() const {
  //---------------------------------------------------------------------
...
Double_t ExtTrackPar::Eta() const { 
  // return pseudorapidity

  return -TMath::Log(TMath::Tan(0.5 * Theta())); 
}

Double_t ExtTrackPar::Y() const {
  // return rapidity
...
Bool_t ExtTrackPar::getXYZ(Double_t *r) const {
  //---------------------------------------------------------------------
...
Bool_t ExtTrackPar::getCovarianceXYZPxPyPz(Double_t cv[21]) const {
  //---------------------------------------------------------------------
...
Bool_t 
ExtTrackPar::getPxPyPzAt(Double_t x, Double_t b, Double_t *p) const {
  //---------------------------------------------------------------------
...
Bool_t 
ExtTrackPar::getYAt(Double_t x, Double_t b, Double_t &y) const {
  //---------------------------------------------------------------------
...
Bool_t 
ExtTrackPar::getZAt(Double_t x, Double_t b, Double_t &z) const {
  //---------------------------------------------------------------------
...
Bool_t 
ExtTrackPar::getXYZAt(Double_t x, Double_t b, Double_t *r) const {
  //---------------------------------------------------------------------
...
//_____________________________________________________________________________
void ExtTrackPar::Print(Option_t* /*option*/) const
{
// print the parameters and the covariance matrix

  printf("ExtTrackPar: x = %-12g  alpha = %-12g\n", fX, fAlpha);
...
Double_t ExtTrackPar::getSnpAt(Double_t x,Double_t b) const {
  //
...
Bool_t ExtTrackPar::getDistance(ExtTrackPar *param2, Double_t x, Double_t dist[3], Double_t bz){
  //------------------------------------------------------------------------
...
void  ExtTrackPar::DrawTrack(Float_t magf, Float_t minR, Float_t maxR, Float_t stepR){
  //
  // Draw track line
  //
  if (minR>maxR) return ;
...
//
void ExtTrackPar::FillPolymarker(TPolyMarker3D *pol, Float_t magF, Float_t minR, Float_t maxR, Float_t stepR){
  //
...
Int_t ExtTrackPar::getIndex(Int_t i, Int_t j){
  //
...
void ExtTrackPar::g3helx3(Double_t qfield, 
                                    Double_t step,
                                    Double_t vect[7]) {
/******************************************************************
 *                                                                *
...
Bool_t ExtTrackPar::propagateToBxByBz(Double_t xk, const Double_t b[3]) {
  //----------------------------------------------------------------
...
Bool_t ExtTrackPar::propagateParamOnlyBxByBzTo(Double_t xk, const Double_t b[3]) {
  //----------------------------------------------------------------
...
Bool_t ExtTrackPar::Translate(Double_t *vTrasl,Double_t *covV){
  //
...
void ExtTrackPar::checkCovariance() {
...
Bool_t ExtTrackPar::getXatLabR(Double_t r,Double_t &x, Double_t bz, Int_t dir) const
{
...
Bool_t ExtTrackPar::getXYZatR(Double_t xr,Double_t bz, Double_t *xyz, Double_t* alpSect) const
{
...
Double_t  ExtTrackPar::getParameterAtRadius(Double_t r, Double_t bz, Int_t parType) const
{
  //
...
Bool_t ExtTrackPar::Local2GlobalMomentum(Double_t p[3], Double_t alpha) const {
  //----------------------------------------------------------------
...
Bool_t ExtTrackPar::Local2GlobalPosition(Double_t r[3], Double_t alpha) const {
  //----------------------------------------------------------------
  // This function performs local->global transformation of the
  // track position.
  // When called, the arguments are:
...
Bool_t ExtTrackPar::Global2LocalMomentum(Double_t p[3], Short_t charge, Double_t &alpha) const {
  //----------------------------------------------------------------
...
Bool_t ExtTrackPar::Global2LocalPosition(Double_t r[3], Double_t alpha) const {
