...
void NA6PMatching::propToZMatching(std::vector<NA6PTrack>& tracks, double z, bool outer)
{
  ...
  for (auto& track : tracks) {
    ...
    if (outer) {
      if (!Propagator::Instance()->propagateToZ(track.getOuterParam(), z, mTrackFitter->getPropOpt())) {
        track.getOuterParam().invalidate();
      }
    } else {
      if (!Propagator::Instance()->propagateToZ(track, z, mTrackFitter->getPropOpt())) {
        track.invalidate();
      }
    }
  }
...
void NA6PMatching::prefilterTracks()
{
  ...
  for (int i = 0; i < (int)hVerTelTrackPtr->size(); ++i) {
    ...
    if (track.getNHits() >= mRecoParam->mtMinVTHits) {
  ...
  std::sort(mSelIDVT.begin(), mSelIDVT.end(), [&](const int i, const int j) { return (*hVerTelTrackPtr)[i].getQ2Pxz() < (*hVerTelTrackPtr)[j].getQ2Pxz(); });
...
bool NA6PMatching::fitAndStoreMatchedTrack(const NA6PTrack& vtTrk, const NA6PTrack& msTrk, float chi2Match)
{
  ...
  if (mRecoParam->mtDoOutwardInwardFit) {
    ...
    if (chi2Out < 0 || chi2Out >= mRecoParam->mtMaxChi2NormRefit * ndf) {
      LOGP(warn, "Forward fit of matched track failed, skipping");
  ...
  if (chi2Inw < 0 || chi2Inw >= mRecoParam->mtMaxChi2NormRefit * ndf) {
    LOGP(warn, "Inward fit of matched track failed, skipping");
    mMatchedTracks.pop_back();
    return false;
  }
