                                               ...
                                               const ClusterType& clu)
{
  ...
  float meas[2] = {clu.getX(), clu.getY()};
  float measErr2[3] = {clu.getSigXX(), clu.getSigYX(), clu.getSigYY()};
  ...
  for (int iLayer = startLayer; iLayer != endLayer; iLayer += step) {
    ...
    for (auto& cand : current) {
      ...
      for (int jCe = firstIndex[layToSearch]; jCe < lastIndex[layToSearch]; ++jCe) {
        ...
        float chi2a = computeTrackToClusterChi2(fitNext, cluRef);
        ...
        float chi2b = computeTrackToClusterChi2(fitRef, cluNext);
  ...
  if (requiredClus > 0) {
    if (nSelected > 0) {
      LOGP(info, "{} with {} clus: Fraction of good = {} / {} = {}  --- average clus = {}",
           label.c_str(), requiredClus, nGood, nSelected, static_cast<float>(nGood) / static_cast<float>(nSelected),
    ...
    if (nFound > 0)
      LOGP(info, "Fraction of good {} = {} / {} = {}  --- average clus = {}",
           label.c_str(), nGood, nFound, static_cast<float>(nGood) / static_cast<float>(nFound),
