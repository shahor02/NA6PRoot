...
class MagneticFieldRegion
{
 public:
  ...
  {
    return
      xyz[2]>=mBoxPos[2][0] && xyz[2]<=mBoxPos[2][1] &&
  ...
  bool getField(const T* xyz, T* bxbybz) const
  {
    ...
    int i = findGridIndex(xyz[0], mXMin, mDXI, mNX);
    int j = findGridIndex(xyz[1], mYMin, mDYI, mNY);
    int k = findGridIndex(xyz[2], mZMin, mDZI, mNZ);
...
private:
  ...
  int findGridIndex(float value, float minValue, float stepInv, int numSteps) const {
    return int((value - minValue) * stepInv);
  }
